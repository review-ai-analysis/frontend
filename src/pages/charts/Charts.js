...
class Charts extends React.Component {
	constructor(props) {
		super(props);
		this.state = { response: [], labels: [], banks: {
			'gpb': 'Газпромбанк',
			'alfa': 'Альфа-Банк',
	...
	componentDidMount() {
		axios.get("https://reviews-ai.ru/api/v1/getstats").then(r => { this.setState({ response: r.data }) });
	...
	render() {
		...
		return (
			<div className={s.widgetClass}>
				...
				<Line data={data} className={s.animCharts} />
			</div>
		);
	}
}
